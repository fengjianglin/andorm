...
public class DexFile {
	...
	public void addClass(File classFile) {
		...
		final ClassDefItem cdi = CfTranslator.translate(classFile.getName(),
				FileUtils.readFile(classFile), cf_options, dex_options);
		file.add(cdi);
	}

	public void addClass(String className, byte[] classBytes) {
		String filePath = className.replace(".", File.separator) + ".class";
		final CfOptions cf_options = new CfOptions();
		final ClassDefItem cdi = CfTranslator.translate(filePath, classBytes,
				cf_options, dex_options);
		file.add(cdi);
	}
